...
IMAGE_MAPPING = {
    ...
    "AE03": "brand-image-prototype-4-dark.webp",
    "AE04": "Construction-CCEED-China-0644_with_overlay.webp",
    "AE05": "fy27-water-image-02.webp"
}
...
# Brand Header Layout
header_html = '<div style="display:flex;align-items:end;justify-content:space-between;flex-wrap:wrap;margin:12px 0 16px;gap:16px;"><div><div style="font-size:.85rem;color:#FFFF00;letter-spacing:.12em;text-transform:uppercase;font-weight:700;">IFM Maturity Assessment</div><div style="font-size:2.0rem;font-weight:700;color:#FFFFFF;letter-spacing:-.03em;">建築・設備 BIM成熟度アセスメント</div></div><div style="font-size:.85rem;color:#D5D5CB;">for Autodesk Design &amp; Make workflows</div></div>'
            ...
            for step_idx in [st.session_state.current_step]:
                ...
                if not skip:
                    ...
                    with col_s2:
                        ...
                        st.slider("将来の目標成熟度 (To-Be)", min_value=1.0, max_value=5.0, step=0.5, key=tobe_key, disabled=(step_idx < st.session_state.current_step))
                        st.markdown("</div>", unsafe_allow_html=True)
                else:
