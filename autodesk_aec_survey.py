...
def generate_pdf_report_bytes(res_name, res_email, res_team, res_exp, answers_list, gaps_sorted, proposals_dict, survey_title):
    ...
    if display_count == 0:
        pdf.set_font("NotoSansJP", size=10)
        pdf.set_text_color(255, 120, 0)
        pdf.cell(0, 6, "■ Autodesk Architecture, Engineering & Construction Collection (AEC)", ln=True)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("NotoSansJP", size=8.5)
        pdf.multi_cell(180, 5, "全体的な成熟度はすでに非常に高い水準にあります。AECコレクションパッケージをご活用いただくことで、設計から施工、運用までの建築ワークフロー全体を一貫してデジタル化できます。")
        pdf.set_text_color(29, 145, 208)
        pdf.cell(0, 5, "製品詳細リンク: https://www.autodesk.com/collections/design-lifecyle-collection/overview", ln=True)
        
    return pdf.output()

# Load AEC Questions
def get_aec_questions():
    survey_id = None
    try:
    ...
    if DATA_JSON.exists():
        with open(DATA_JSON, "r", encoding="utf-8") as f:
            data = json.load(f)
        return pd.DataFrame(data.get("aec_questions", [])), "default", None
    return pd.DataFrame(), "default", None

q_df, active_survey_id, client_name = get_aec_questions()
...
# Image Mapping for AEC
IMAGE_MAPPING = {
    "AE01": "fy27-aec-forma-industry-cloud-imagery.webp",
    "AE02": "brand-image-prototype-1-dark.webp",
    "AE03": "Construction-CCEED-China-0644_with_overlay.webp",
    "AE04": "fy27-water-image-02.webp",
    "AE05": "fy27-dm-digital-factory-campaign-visual-01.webp",
    "AE06": "Construction-CCEED-China-0644_with_overlay.webp",
    "AE07": "brand-image-prototype-4-dark.webp",
    "AE08": "fy27-dm-fusion-industry-cloud-imagery.webp"
}
...
with tabs[0]:
    ...
    with col_left_form:
        if st.session_state.is_submitted:
            ...
            PRODUCT_PROPOSALS = {
                "AE01": {
                    "title": "AEC初期計画・敷地・環境分析クラウド - Autodesk Forma",
                    "desc": "敷地ボリューム検討や日影・風環境などの初期計画フェーズに大きなGapが見られます。Autodesk Formaを活用することで、敷地情報に基づき日影・風向・騒音をクラウド上でAIにより即時シミュレーションし、手戻りの少ない建築計画を素早く決定できます。",
                    "url": "https://www.autodesk.com/products/forma/overview"
                },
                "AE02": {
                    "title": "プロフェッショナルBIM設計プラットフォーム - Autodesk Revit",
                    "desc": "意匠・構造・設備における詳細設計プロセスの3D BIM化に課題があります。Revitの導入により、単一の3D BIMデータ内でパラメータ付きの図面と集計表が完全同期し、図面作成業務の飛躍的な生産性向上を実現します。",
                    "url": "https://www.autodesk.com/products/revit/overview"
                },
                "AE03": {
                    "title": "3Dモデル統合検証・干渉チェック - Autodesk Navisworks",
                    "desc": "複数領域のモデル重ね合わせや干渉検出にGapが見られます。Navisworksを導入し、意匠・構造・設備、さらに施工会社の異なるデータを一括統合して自動干渉検出を実行することで、施工段階での手戻りをゼロに近づけます。",
                    "url": "https://www.autodesk.com/products/navisworks/overview"
                },
                "AE04": {
                    "title": "共通データ環境（CDE）と設計コラボレーション - Autodesk Construction Cloud (ACC)",
                    "desc": "関係者間のデータ共有や、承認ワークフローのクラウド管理に大きな改善余地があります。ACC DocsおよびDesign Collaborateにより、ISO 19650に準拠したセキュアなBIM共有とリアルタイムの協調設計を実現します。",
                    "url": "https://www.autodesk.com/products/autodesk-construction-cloud/overview"
                },
                "AE05": {
                    "title": "クラウド型数量算出・積算ツール - ACC Takeoff",
                    "desc": "BIMモデルからの数量算出（QTO）および積算業務のデジタル化に乖離が見られます。ACC Takeoffを活用することで、2D図面と3D BIMモデルの双方から高精度な数量を半自動で抽出し、見積作成を劇的に高速化します。",
                    "url": "https://www.autodesk.com/products/autodesk-takeoff/overview"
                },
                "AE06": {
                    "title": "現場管理・デジタル施工施工管理ツール - ACC Build",
                    "desc": "現場での図面確認、指摘事項・写真管理、および出来形品質管理に課題が見られます。ACC Buildの導入により、モバイル端末から常に最新の図面やチェックリストを参照し、現場とオフィスの連携をリアルタイム化します。",
                    "url": "https://www.autodesk.com/products/autodesk-build/overview"
                },
                "AE07": {
                    "title": "4D施工工程シミュレーション - Autodesk Navisworks",
                    "desc": "施工計画・4D工程シミュレーションの運用状況にGapがあります。Navisworksを用いてBIMオブジェクトと工事スケジュールを連動させ、実際の施工ステップをアニメーション検証することで、工程の不整合を事前に回避します。",
                    "url": "https://www.autodesk.com/products/navisworks/overview"
                },
                "AE08": {
                    "title": "建物アセットデジタルツインプラットフォーム - Autodesk Tandem",
                    "desc": "竣工後の維持管理（FM）段階へのデータ受け渡しやデジタルツイン構築に大きな乖離が見られます。Autodesk Tandemを導入することで、竣工BIMからFMアセットデータを自動整理し、IoT連携による予知保全システムを構築できます。",
            ...
            if display_count == 0:
                st.markdown(
                    ...
                    <div class="proposal-card">
                        ...
                        <div class="proposal-desc">お客様のBIM推進レベルはすでに極めて高い水準にあります。Forma、Revit、Navisworks、Civil 3Dを包括したAECコレクションパッケージをご活用いただくことで、設計から施工、運用までの建築ワークフロー全体を一貫してデジタル化できます。</div>
            ...
            with st.spinner("PDFレポートを準備中..."):
                pdf_data = generate_pdf_report_bytes(
                    ...
                    survey_title="建築設計・施工 BIM適性診断"
                )
            
            st.download_button(
                label="診断結果レポート (PDF) をダウンロード",
                data=pdf_data,
                file_name=f"Autodesk_AEC_BIM_Report_{active_survey_id}.pdf",
            ...
            specific_team = st.text_input("部署名・チーム名 (任意)", placeholder="例: 設計部 建築課", value=st.session_state.get("res_team", ""))
